...
from src.algo.WADN import LabelWADNBase
...
class WADN_MLP_Label(LabelWADNBase):
...
class WADN_Conv_Digits_Label(LabelWADNBase):
    def __init__(self, configs):
        """
...
def learn(args):
    ...
    exp_flags = "lr_{:g}_mu_{:g}_gp_{:g}_sem_{:g}_seed_{:d}_{}_shift_labelled_drop_{:g}_percent_{:g}".format(
        args.lr, args.mu, args.gp_coef, args.sem_coef, args.seed, args.alpha_solver, args.drop, args.percent
    )
    ...
    for tar_dom_idx, tar_dom_name in enumerate(data_names):
        ...
        num_drop = int(np.ceil(len(target_y_temps) * args.percent))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--percent", help="few labeled data, 1-percent", type=float, default=0.9)
    parser.add_argument(
        "--mu",
    ...
    parser.add_argument(
        "--sem_coef", help="Coefficent of semantic loss(mu * sem_coef).", type=float, default=0
    )
