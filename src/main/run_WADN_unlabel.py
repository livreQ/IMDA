...
from src.algo.WADN import LabelWADNBase,WANDBase
...
class WADN_Conv_Digits(WANDBase):
    def __init__(self, configs):
        """
...
def learn(args):
    ...
    exp_flags = "lr_{:g}_mu_{:g}_gp_{:g}_sem_{:g}_seed_{:d}_{}_{}_drop_{:g}_percent_{:g}".format(
        args.lr,
        args.mu,
        args.gp_coef,
        args.sem_coef,
    ...
    for tar_dom_idx, tar_dom_name in enumerate(data_names):
        ...
        if args.name in ["amazon"]:  # MLP
            model = WADN_MLP(configs).to(device)
        elif args.name == "digits":  # ConvNet
            model = WADN_Conv_Digits(configs).to(device)
